...
- Single-flight dedup; SwiftData cache (`RecoveryInsight` new type `.aiDailySession` or a dedicated `DailySession` `@Model`).
...
- **RESOLVED (analysis agent): KEEP — high confidence, don't hedge.** The prior work is merged and LIVE in the decision path (`TrainingViewModel.apiClient:210`, `AIProgramPlanner.reconcile` with `[0.5,1.1]` clamp `:103-124`, `PredictionLog`, `TrainingOutcomeEvaluator` all present). Superseding = deleting working, tested, safety-clamped, integrated code to rebuild the gym loop inside the LLM — strictly worse. The daily brain sits ABOVE: it decides "today = gym," then delegates set/rep/weight/progression to the existing engine + reconcile + prediction spine. The `PredictionLog` spine becomes the accuracy check the new floor needs.
- **CADENCE CAVEAT (review):** the existing `AIProgramPlanner` runs WEEKLY (`hydrateWeekWithAI`, once/ISO-week); the new brain runs DAILY. Two AI loops at two cadences. **The spec MUST define their relationship** or they desync. Resolution: the weekly planner sets the week's SHAPE (which days are soccer-emphasis vs physique-emphasis vs recovery, around matches); the daily brain executes WITHIN that day's slot, adjusting modality/intensity to today's readiness. Weekly = strategy, daily = tactics. The daily brain never contradicts the week's emphasis; it only adjusts how hard / which modality within it.
...
- **Files (new):** `DailyReadinessCoach.swift`, `TrainingSafetyFloor.swift`, `DailySession` model, daily prompt.
...
- **Emphasis weeks, not emphasis days:** the weekly planner (§8 cadence) sets a soccer-emphasis or physique-emphasis bias for the week; the daily brain prescribes within it. You cannot maximize both in the same microcycle — the system picks a primary per block and protects the other at maintenance.
...
- **Does the weekly AIProgramPlanner (existing) get refactored to consume the new block-emphasis, or run parallel?** §8 says strategy-vs-tactics but not the integration mechanics. [before D3]
