...
final class NutritionTabViewModelTests: XCTestCase {
    ...
    func testMacroMath_planning_sumsWithin10kcal() {
        viewModel.todayMeals = [makePlanned(meal: 1, p: 40, c: 50, f: 15, kcal: 535, status: .eaten)]
    ...
    func testRecoveryAdjustedTarget_addsCaloriesOnHighRecovery() {
        viewModel.todayRecovery = WhoopRecoveryData(
    ...
    func testScheduleMealReminders_skipsAlreadyEaten() {
        ...
        XCTAssertEqual(mock.scheduledNotifications.filter { $0.category == "meal_reminder" }.count, 1, "Only the planned meal should be scheduled")
    }
