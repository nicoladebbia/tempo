...
struct StrainDetailView: View {
        ...
        VStack(alignment: .leading, spacing: TempoSpacing.md) {
            ...
            VStack(spacing: TempoSpacing.sm) {
                if let totalCal = viewModel.todayRecovery?.caloriesBurned, totalCal > 0 {
                    ...
                    // Estimate active vs basal (rough 50/50 for active users)
    ...
    private var strainTrendSection: some View {
        VStack(alignment: .leading, spacing: TempoSpacing.md) {
            sectionHeader("Strain Trends")

            if viewModel.strainChartData.count >= 2 {
                TempoLineChart(
                    data: [
                        TempoLineChartData(
                            id: "strain",
    ...
    private var strainClassification: String {
        guard let strain = viewModel.todayRecovery?.strain else { return "--" }
        switch strain {
    ...
    private var strainColor: Color {
        guard let strain = viewModel.todayRecovery?.strain else { return Color.tempoTextTertiary }
        switch strain {
