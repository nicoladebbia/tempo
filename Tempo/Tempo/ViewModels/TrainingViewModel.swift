...
final class TrainingViewModel {
    ...
    func loadToday(modelContext: ModelContext) async {
        ...
        // Check for any persisted plan for today.
        ...
        if let existing {
            ...
            } else if let canonical = weekPlanForToday, canonical.type != existing.type {
                // Persisted plan disagrees with the Week Plan (e.g. user changed
                // Football Days, or it was generated by the older split-only
                // path). Replace it with the canonical Week Plan version so the
                // Today card and Week Plan show the same workout.
            ...
            // Fallback: Week Plan generator returned nothing for today (shouldn't
            // happen, but don't crash). Use the single-day generator as a
            // safety net.
