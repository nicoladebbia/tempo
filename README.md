...
- **Honest auth caveat:** the access-token signing key is currently HMAC-SHA256, not the ES256/P-256 the docs target (`configure.swift` carries the comment). And `POST /subscription/verify` currently *trusts* the client-supplied StoreKit transaction (TODO in code) — the Apple-driven server-notification webhook path *is* JWS-verified against Apple Root CA G3, but the client path is a pre-launch security gap. Called out here rather than overstated.
